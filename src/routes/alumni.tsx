...
function AlumniTickerCard() {
  ...
  return (
    <div
      className="relative w-full rounded-2xl border border-foreground/12 bg-background/40 backdrop-blur-sm overflow-hidden flex flex-col p-3 sm:p-4"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
      style={{ boxShadow: `0 20px 60px -30px ${accent}55` }}
    >
      ...
      <h2 className="mt-1.5 text-base sm:text-lg font-semibold tracking-tight text-foreground">
        IIT Bombay Connect
      </h2>

      <div key={idx} className="mt-2 animate-in fade-in slide-in-from-bottom-2 duration-500">
        <div className="flex items-center gap-2">
          <span
            className="text-[9px] font-semibold tracking-[0.28em] uppercase px-2 py-0.5 rounded-full border"
            style={{
        ...
        <div className="mt-1.5 text-sm font-semibold text-foreground leading-snug line-clamp-1">
          {item.topic}
        </div>
      ...
      <div className="mt-2 border-t border-foreground/10 pt-2 text-[10px] text-foreground/60 leading-snug line-clamp-2">
        Visit the official IIT Bombay Alumni Association site for better engagement.
      </div>
    </div>
  );
