...
function VideoPiP({ videos }: { videos: VideoItem[] }) {
  ...
  return (
    <div className="h-full w-full rounded-xl overflow-hidden border border-white/20 bg-background shadow-[0_0_40px_-10px_rgba(0,0,0,0.7)] flex flex-col">
      ...
      <div className="px-3 py-1.5 text-xs text-white/70 truncate bg-background/60 shrink-0">{v.title}</div>
    </div>
  );
}
  ...
  return (
    <div className="relative h-full w-full overflow-hidden flex flex-col">
      ...
      <div className="relative h-[22%] w-full shrink-0 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-black/10" />
        <div className="relative z-10 h-full flex flex-col justify-between p-4 sm:p-6">
          <button onClick={onBack} className="self-start inline-flex items-center gap-2 rounded-lg border border-white/20 bg-background/30 backdrop-blur px-4 py-2 text-sm font-medium text-white/90 hover:border-primary/60 transition-colors">
            <I.Back /> Back
          </button>
          <div>
            <div className={`label-caps text-[11px] mb-1 ${TINT_TEXT[section.tint]}`}>{section.eyebrow}</div>
